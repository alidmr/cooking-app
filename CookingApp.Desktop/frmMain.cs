...
namespace CookingApp.Desktop
{
    public partial class frmMain : Form
    {
        ...
        private async void btnAsync_Click(object sender, EventArgs e)
        {
            ...
            //burada YumurtalariKirAsync() işleminden sonra ContinueWith() diyerek içindeki metodların devam etmesi söylüyoruz
            var yumurtaTaskGroup = await YumurtalariKirAsync()
                .ContinueWith(async _ =>
                {
                    //burası 20 saniye sürse bile YumurtalariKirAsync() 1 saniye sürdüğünü varsayalım 1 saniye sonra altaki işlemlere devam eder
                    await YumurtalariCirpAsync();
                    await TuzEkleAsync();
                }, TaskScheduler.FromCurrentSynchronizationContext()); //YumurtalariCirpAsync() TuzEkleAsync() bunlar çalışırken başka bir thread oluşturma şanki threaded çalıştır diyoruz
            ...
            await Task.WhenAll(yumurtaTaskGroup, ocakTaskGroup); //bu satır çalıştığı zaman hem yumurtaTaskGroup'un hemde ocakTaskGroup'un bitmiş olması gerekiyor.ondan sonra alt satıra geçiyor
        ...
        {
            Task.Delay(500).Wait();  //burada wait() bekle dediğim zaman uı therad i kitleniyor
        ...
        {
            await Task.Delay(500);  //uı thread i kitlemeden await ile bekle
            AddLog("Yumurtalar Kırıldı");
            AdjustButtons(1);
        }
